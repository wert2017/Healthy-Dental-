...
def generar_historia_clinica_pdf(paciente, sucursal=None):
    ...
    if logo_path:
        try:
            # Logo bajado 5mm mas (y=page_height-60)
            c_header.drawImage(logo_path, -20, page_height - 60, width=160, height=50, preserveAspectRatio=True, mask='auto')
        except Exception:
            pass
    ...
    for i in range(len(reader.pages)):
        ...
        if i in (1, 2):
            ...
            # Ajuste adicional para página 2 (índice 1): desplazar 3mm a la izquierda (aprox 8.5 puntos)
            if i == 1:
                tx -= 8.5
